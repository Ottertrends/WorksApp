...
import { deliveryResult, sendTelnyxWhatsAppText } from "@/lib/whatsapp/telnyx";
import { after, NextResponse } from "next/server";
import { createPublicKey, verify } from "node:crypto";

import { processContractorMessage } from "@/lib/agent/contractor-agent";
import { createSupabaseAdminClient } from "@/lib/supabase/admin";
import { logBotEvent } from "@/lib/whatsapp/bot-events";
...
async function downloadTelnyxMedia(url: string): Promise<Response> {
  ...
  if (parsed.protocol !== "https:" || parsed.username || parsed.password) throw new Error("Unsupported media URL");
  const headers = parsed.hostname === "api.telnyx.com" ? { Authorization: `Bearer ${getTelnyxApiKey()}` } : undefined;
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (!response.ok && headers) response = await fetch(url, { signal: AbortSignal.timeout(15000) });
...
export async function POST(request: Request) {
  ...
  if (eventType !== "message.received") {
    const statusPayload = (data ?? {}) as { to?: unknown; errors?: unknown };
    const result = eventType === "message.sent" || eventType === "message.finalized"
      ? deliveryResult(statusPayload) : "ignored-event";
    const providerId = data?.id ?? null;
    const admin = createSupabaseAdminClient();
    const { data: outbound } = providerId ? await admin.from("messages").select("user_id")
      .eq("whatsapp_message_id", providerId).eq("direction", "outbound").eq("provider", "telnyx").maybeSingle()
      : { data: null };
    const saved = await logWebhookEvent({ eventType, result, userId: outbound?.user_id,
      messageId: providerId, raw: payload });
    if (result === "delivery-failed") console.error("[whatsapp-delivery] failed", { messageId: providerId });
    return NextResponse.json({ ok: saved, result }, { status: saved ? 200 : 503 });
  }
...
export async function handleTelnyxInbound(data: TelnyxPayload | null, raw: TelnyxWebhook) {
  ...
  for (const [mediaIndex, item] of uniqueMedia.entries()) {
    try {
      const saved = await persistTelnyxMedia({ admin, userId: workspace.workspaceUserId, messageId: mediaIndex ? `${messageId}:${mediaIndex}` : messageId, media: item, caption });
  ...
  const { error: contentError } = await admin.from("messages").update({
    content: commandText,
    message_type: persistedMediaTypes.includes("video") ? "video" : persistedMediaTypes.length ? "image" : "text",
  }).eq("id", inboundId).eq("user_id", userId);
  if (contentError) throw new Error("Unable to persist inbound content");
  ...
  let reply: string;
  try {
    const agentResult = await processContractorMessage(userId, commandText, history, workspace.workspaceUserId);
  ...
  try {
    const outboundMessageId = await sendTelnyxWhatsAppText(to, from, reply);
    const { error: saveError } = await admin.from("messages").insert({
      user_id: userId, project_id: null, direction: "outbound", content: reply,
      message_type: "text", whatsapp_message_id: outboundMessageId,
      sender_phone_e164: to, recipient_phone_e164: from, provider: "telnyx", processed: true,
    });
    if (saveError) {
      console.error("[whatsapp-delivery] accepted-but-not-persisted", { messageId: outboundMessageId });
      await logWebhookEvent({ eventType: "message.sent", result: "accepted-but-not-persisted",
        userId, messageId: outboundMessageId, error: saveError.message });
    }
    const { error: processedError } = await admin.from("messages").update({ processed: true }).eq("id", inboundId);
    if (processedError) console.error("[whatsapp-delivery] processed-update-failed", { messageId });
    const { error: trimError } = await admin.rpc("trim_user_messages", { p_user_id: userId, p_keep: 50 });
    if (trimError) console.warn("[whatsapp-delivery] history-trim-failed");
